    ...
    register_t *retval)
{
	...
	// XXX Must reset FPU state, if applicable
	// For now, just unset the pcb FPU flag
	p->p_addr->u_pcb.pcb_flags &= ~PCB_FPU;

	tf = p->p_addr->u_pcb.pcb_tf;

	// XXX What other registers might we need to initialize?
...
void
initriscv(struct riscv_bootparams *abp)
{
	vaddr_t vstart, vend;
